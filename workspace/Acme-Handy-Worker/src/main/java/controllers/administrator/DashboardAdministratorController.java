...
public class DashboardAdministratorController extends AbstractController {
	...
	public ModelAndView dashboard() {
		...
		for (final Customer c : moreFixUpTasks)
			customerNames.add(c.getName() + " " + c.getSurname());

		res.addObject("moreFixUpTasks", customerNames);

		final Collection<HandyWorker> moreApplications = this.hwService.handyWorkersWithMoreApplications();
		final List<String> hwNames = new ArrayList<>();
		for (final HandyWorker h : moreApplications)
			hwNames.add(h.getName() + " " + h.getSurname());

		res.addObject("moreApplications", hwNames);
